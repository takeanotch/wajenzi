...
const OurTeam = () => {
  ...
  const nextSlide = () => {
    ...
    if (isMobile) {
      ...
      setCurrentIndex((prev) => {
        const next = prev + 2;
        return next >= teamMembers.length ? 0 : next;
      });
    }
  };
  ...
  const prevSlide = () => {
    ...
    if (isMobile) {
      ...
      setCurrentIndex((prev) => {
        const next = prev - 2;
        return next < 0 ? teamMembers.length - (teamMembers.length % 2 === 0 ? 2 : 1) : next;
      });
    }
  };
  ...
  // Obtenir les membres visibles
  const getVisibleMembers = () => {
    if (isMobile) {
      // Sur mobile: 2 membres avec la même taille, mais le 2ème est coupé
      const members = [];
      
      // Premier membre (complètement visible)
      members.push({
      ...
      // Deuxième membre (même taille mais coupé à droite)
      ...
      // Sur desktop: 2 membres complets côte à côte
      const members = [];
      ...
      const secondIndex = (currentIndex + 1) % teamMembers.length;
      members.push({
        ...teamMembers[secondIndex],
        isPartial: false,
        isCut: false
      });
    ...
    <div className="min-h-screen py-12 px-4 md:px-8">
      <div className="max-w-4xl mx-auto">
        ...
        <div className="relative">
          ...
          {/* Container avec défilement limité */}
          <div className={`overflow-hidden ${isMobile ? 'px-4' : ''}`}>
            <motion.div 
              className={`flex gap-4 md:gap-6 ${isMobile ? 'w-[140%]' : ''}`}
              variants={containerVariants}
              initial="hidden"
              whileInView="visible"
              viewport={{ amount: 0.1 }}
                ...
                {visibleMembers.map((member, index) => (
                  <motion.div
                    ...
                    className={`flex-shrink-0 ${
                      isMobile 
                        ? 'w-[85%]' // Même taille pour les deux cartes sur mobile
                        : 'w-1/2'
                    } ${member.isCut ? 'opacity-90' : ''}`}
                    variants={cardVariants}
                    whileHover="hover"
                    custom={direction}
                    initial="hidden"
                  ...
                  >
                    <div className="bg-white border border-gray-100 rounded-xl p-6 h-full">
                      ...
                      <motion.div 
                        ...
                        whileHover="hover"
                      >
                        <div className={`overflow-hidden rounded-full border border-gray-100 ${
                          isMobile 
                            ? 'w-32 h-32' 
                            : 'w-40 h-40'
                        }`}>
        ...
        <div className="flex justify-center gap-2 mt-8">
          {Array.from({ 
            length: isMobile ? teamMembers.length : Math.ceil(teamMembers.length / 2) 
          }).map((_, index) => (
            <button
              key={index}
              onClick={() => goToSlide(index)}
              ...
              <motion.div
                className={`w-2 h-2 rounded-full ${
                  isMobile
                    ...
                    : currentIndex === index * 2 ? 'bg-gray-900' : 'bg-gray-300'
                }`}
                whileHover={{ scale: 1.5 }}
                animate={{ 
                  scale: (isMobile ? index === currentIndex : currentIndex === index * 2) ? 1.2 : 1,
                }}
                transition={{ duration: 0.2 }}
              />
            </button>
        ...
        >
          <p className="text-xs text-gray-500">
            {isMobile 
              ? `${currentIndex + 1} / ${teamMembers.length}`
              : `${Math.floor(currentIndex / 2) + 1} / ${Math.ceil(teamMembers.length / 2)}`
            }
          </p>
        </motion.div>
      ...
      <style jsx global>{`
        ...
        * {
        ...
        /* Pour créer l'effet de coupure sur mobile */
