...
export default function JobDetail() {
  ...
  useEffect(() => {
    const incrementViews = async () => {
      if (jobId && !loading && job) {
        try {
          // Appeler la fonction RPC pour incrémenter les vues
          const { error } = await supabase.rpc('increment_job_offer_views', {
            job_id: jobId
          })
          
          if (error) {
            console.error('Error incrementing views:', error)
            // Fallback: incrémenter localement si la RPC échoue
            setJob((prev: { views_count: any }) => prev ? {
              ...prev,
    ...
    // Délai pour éviter les vues multiples
    const timer = setTimeout(() => {
      incrementViews()
    }, 2000) // 2 secondes de délai

    return () => clearTimeout(timer)
  }, [jobId, loading, job, supabase])
  ...
  const applyForJob = () => {
    if (userRole === 'seeker') {
      ...
      router.push('/signup?role=seeker&redirect=' + encodeURIComponent(`/apply/${jobId}`))
    }
  }
          ...
          <div className="flex flex-col lg:flex-row lg:items-start justify-between gap-6">
              ...
              <div className="flex items-start gap-4">
                ...
                <div>
                  ...
                  <div className="flex flex-wrap items-center gap-4 mb-3">
                    ...
                    <span className="flex items-center gap-1 text-gray-500 text-sm">
                      <Clock className="w-4 h-4" />
                      {job.views_count || 0} vue{job.views_count !== 1 ? 's' : ''}
                    </span>
                  </div>
                </div>
              </div>
            ...
            <div className="flex flex-col gap-3">
              <button
                ...
                className="px-6 py-3 bg-black text-white hover:bg-gray-800 transition-colors"
              >
                {userRole === 'seeker' ? 'Postuler maintenant' : 'Créer un compte pour postuler'}
              </button>
          ...
          <div className="space-y-6">
            ...
            <div className="border border-gray-200 p-5 bg-gray-50">
              ...
              <button
                ...
                className="w-full py-3 bg-black text-white hover:bg-gray-800 transition-colors"
              >
                {userRole === 'seeker' ? 'Postuler maintenant' : 'Créer un compte candidat'}
              </button>
            </div>
          </div>
        </div>
